<script lang="ts">
	...
	function deleteBot(botId: string) {
		...
		const serializable = savedBots.map((b) => ({
			...
			category: b.category,
			webSearchEnabled: b.webSearchEnabled
		}));
									...
									{:else}
										{#each botsInCategory as bot (bot.id)}
												...
												<div class="flex-1 min-w-0">
													<div class="flex items-center gap-1">
														...
														{#if bot.webSearchEnabled}
															<svg class="w-3.5 h-3.5 text-blue-500 dark:text-blue-400 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
											...
											{#if editingBotId === bot.id}
												<div use:portal class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[200] p-4">
														...
														<div class="flex-1 overflow-y-auto px-6 py-4 min-h-0">
															...
															<div class="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
																<label class="relative inline-flex items-center cursor-pointer">
																	<input
																		type="checkbox"
																		checked={bot.webSearchEnabled || false}
																		on:change={(e) => (bot.webSearchEnabled = e.currentTarget.checked)}
																		class="sr-only peer"
																	/>
